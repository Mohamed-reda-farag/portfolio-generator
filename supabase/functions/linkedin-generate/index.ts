...
function buildPrompt(
  ...
  // Fix 6: viralTag removed from prompt — applied as postprocess in callGroqAPI()
...
عمر الحساب: ${accountAge} سنة/سنوات
اللغات الرئيسية: ${topLangs.join("، ")}

المستودعات الأهم:
${JSON.stringify(repoSummaries, null, 2)}

نقاط القوة والضعف (من الحساب التلقائي):
- درجة README: ${scoreData.breakdown.readme_quality}/20
- نشاط الـ commits: ${scoreData.breakdown.commit_frequency}/20
- وصف المستودعات: ${scoreData.breakdown.repo_descriptions}/20
- تنوع المشاريع: ${scoreData.breakdown.project_diversity}/20
- اكتمال الملف الشخصي: ${scoreData.breakdown.profile_completeness}/20

أنشئ JSON بالبنية التالية بالضبط:
{
  "weak_points": ["نقطة ضعف 1 محددة جداً", "نقطة ضعف 2", "نقطة ضعف 3"],
  "headline": "عنوان LinkedIn مميز (ما بين 100-220 حرف)",
  "about": "قسم About المهني الكامل (300-500 كلمة)",
  "posts": [
    {
      "repo_name": "اسم المستودع الدقيق",
      "post_content": "منشور LinkedIn كامل وجاهز للنشر (150-200 كلمة)"
    }
  ]
}

قواعد صارمة:
- weak_points: 3 فقط، محددة جداً — "أضف وصفاً للمستودع X" لا "حسّن التوثيق"
- headline: BANNED words — passionate، love coding، enthusiastic، dedicated
- posts: منشور واحد لكل مستودع بالترتيب — استخرج التفاصيل الحقيقية من readme_snippet
- كل منشور يبدأ بـ hook مختلف تماماً
- about: لا تبدأ بـ "أنا" أو اسم المطور`;

  // ── English User Prompt ──
  const promptEn = `Based on the following GitHub data, create professional LinkedIn content that drives real engagement:
...
Account Age: ${accountAge} year(s)
Primary Languages: ${topLangs.join(", ")}

Top Repositories:
${JSON.stringify(repoSummaries, null, 2)}

Score breakdown (from deterministic calculation):
- README Quality: ${scoreData.breakdown.readme_quality}/20
- Commit Activity: ${scoreData.breakdown.commit_frequency}/20
- Repo Descriptions: ${scoreData.breakdown.repo_descriptions}/20
- Project Diversity: ${scoreData.breakdown.project_diversity}/20
- Profile Completeness: ${scoreData.breakdown.profile_completeness}/20

Generate EXACTLY this JSON structure:
{
  "weak_points": ["specific weak point 1", "specific weak point 2", "specific weak point 3"],
  "headline": "LinkedIn headline (100-220 characters)",
  "about": "Full LinkedIn About section (300-500 words)",
  "posts": [
    {
      "repo_name": "exact repo name from the list",
      "post_content": "complete LinkedIn post ready to publish (150-200 words)"
    }
  ]
}
...
// Fix 6: viralTag applied here as postprocess — not injected into prompt
...
async function callGroqAPI(
  ...
  return {
    ...
    // Fix 6: viralTag appended here after AI response, not injected into prompt
    posts: parsed.posts.map((p: { repo_name?: unknown; post_content?: unknown }) => ({
      repo_name: String(p.repo_name || "").trim(),
      post_content: String(p.post_content || "").trim() + VIRAL_TAG,
    })),
...
// Fix 7: already using serve() from std — migrated to Deno.serve (removed std import)
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (token && token !== (Deno.env.get("SUPABASE_ANON_KEY") || "")) {
      try {
        ...
        if (userId) {
          ...
          // Fix 1: rate limit scoped to function_type='linkedin' only
          const { count } = await supabase
            .from("ai_generations")
            .select("*", { count: "exact", head: true })
            .eq("user_id", userId)
            .eq("function_type", "linkedin")
    ...
    // Fix 2: function_type: 'linkedin' written immediately on first status update
    // to override the frontend's default 'portfolio' insert before rate limit window closes
    await updateGenerationStatus(supabase, generationId, "processing", {
      github_data: body.githubData,
      function_type: "linkedin",  // corrects the default 'portfolio' set by frontend insert
    });
