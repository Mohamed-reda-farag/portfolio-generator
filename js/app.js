...
function initAnimations() {
  ...
  // Eyebrow
  heroTl.to('[data-anim="fade"]:first-of-type, .hero__eyebrow [data-anim]', {
    opacity: 1,
    duration: 0.6,
    delay: 0.1,
  });
  ...
  gsap.utils.toArray('[data-anim]:not(.hero__title):not(.hero__sub):not(.hero__timer):not(.stats-strip):not(.hero-form-card)').forEach(el => {
    ...
    ScrollTrigger.create({
      ...
      onEnter: () => {
        statValues.forEach(el => {
          ...
          const prefix = el.querySelector('span')?.textContent || '';

          // Rebuild without inner span to animate cleanly
          // then restore
          const counter = { val: 0 };
          gsap.to(counter, {
            val: finalNum,
            ...
            onUpdate: () => {
              ...
              el.innerHTML = display + suffix.replace(prefix, '') +
...
/* ─────────────────────────────────────────────────────────────────
   GLOBAL UTILITIES (available across all pages)
───────────────────────────────────────────────────────────────── */
